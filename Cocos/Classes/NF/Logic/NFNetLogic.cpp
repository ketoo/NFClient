...
bool NFNetLogic::Init()
{
    return true;
}

bool NFNetLogic::Shut()
{
    return true;
}

bool NFNetLogic::ReadyExecute()
{
    return true;
}

bool NFNetLogic::Execute()
{
...
bool NFNetLogic::AfterInit()
{
	NFLogicBase::AfterInit();
	g_pNetClientModule->AddEventCallBack(NF_SERVER_TYPES::NF_ST_NONE, this, &NFNetLogic::OnSocketEvent);
	g_pNetClientModule->AddReceiveCallBack(NF_SERVER_TYPES::NF_ST_NONE, this, &NFNetLogic::OnMsgRecive);

	g_pNetLogic->AddReceiveCallBack(NFMsg::EGMI_EVENT_RESULT, this, &NFNetLogic::OnEventResult);

    return true;
}

void NFNetLogic::ConnectServer(const std::string strIp, int nPort)
{
...
void NFNetLogic::SendToServerByPB(const uint16_t nMsgID, google::protobuf::Message& xData)
{
	if(m_bSocketReady)
	{
...
NFINetClientModule *NFNetLogic::GetNetModule()
{	
...
void NFNetLogic::OnSocketEvent(const NFSOCK nSockIndex, const NF_NET_EVENT eEvent, NFINet* pNet)
{
...
void NFNetLogic::OnEventResult(const NFSOCK nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
	NFGUID nPlayerID;
...
void NFNetLogic::OnMsgRecive(const NFSOCK nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
