...
NFUISelectServer::NFUISelectServer()
{
}
...
bool NFUISelectServer::initLayout()
{	
	...
	g_pLoginLogic->AddEventCallBack(E_LoginEvent_WorldList, this, &NFUISelectServer::OnWorldListEvent);
	g_pLoginLogic->AddEventCallBack(E_LoginEvent_ServerList, this, &NFUISelectServer::OnServerListEvent);
	return true;
}

void NFUISelectServer::initData(void *customData)
{
	g_pLoginLogic->RequireWorldList();
	m_pServerList->removeAllItems();
}

int NFUISelectServer::OnWorldListEvent(const int nEventID, const NFDataList& varDataList)
{
	...
	for(size_t i = 0; i < worldList.size(); i++)
	{
		...
		pButton->addClickEventListener(CC_CALLBACK_1(NFUISelectServer::OnWorldSelected, this));
...
int NFUISelectServer::OnServerListEvent(const int nEventID, const NFDataList& varDataList)
{
	...
	for(size_t i = 0; i < worldList.size(); i++)
	{
		...
		m_pServerList->pushBackDefaultItem();
		ui::Button *pButton = (ui::Button *)m_pServerList->getItem(m_pServerList->getItems().size()-1);
		pButton->addClickEventListener(CC_CALLBACK_1(NFUISelectServer::OnServerSelected, this));
		pButton->setTitleText(kServerInfo.name());
		pButton->setTitleColor(Color3B(0, 0, 0));
	}
...
void NFUISelectServer::OnWorldSelected(Ref* sender)
{
	auto worldList = g_pLoginLogic->GetWorldList();
