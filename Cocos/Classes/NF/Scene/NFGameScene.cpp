...
#include "UI/NFUIChatPanel.h"

NFGameScene::NFGameScene()
{
}
...
bool NFGameScene::initLayout()
{	
	auto map = TMXTiledMap::create("Scene/NFMap.tmx");
	addChild(map);
	map->setPosition(map->getContentSize()*-0.5);
	
    g_pKernelModule->AddClassCallBack(NFrame::Player::ThisName(), this, &NFGameScene::OnObjectClassEvent);

	g_pPlayerLogic->AddEventCallBack(E_PlayerEvent_PlayerMove, this, &NFGameScene::OnPlayerMoveEvent);
	g_pChatLogic->AddEventCallBack(E_ChatEvent_ChatGlobal, this, &NFGameScene::OnPlayerChatEvent);
	...
	NFUIChatPanel::showPanel();

	return true;
}

void NFGameScene::initData(void *pData)
{
}

void NFGameScene::update(float dt)
{
...
bool NFGameScene::onTouchBegan(Touch *touch, Event *unused_event)
{
...
void NFGameScene::onTouchMoved(Touch *touch, Event *unused_event)
{
	Vec2 toPos = this->convertTouchToNodeSpace(touch);
	g_pPlayerLogic->RequireMove(NFVector3(toPos.x, toPos.y, 0));
}

int NFGameScene::OnObjectClassEvent(const NFGUID& self, const std::string& strClassName, const CLASS_OBJECT_EVENT eClassEvent, const NFDataList& var)
{
    if (strClassName == NFrame::Player::ThisName())
    {
        if (CLASS_OBJECT_EVENT::COE_CREATE_FINISH == eClassEvent)
        {
			...
			addChild(pPlayer);
			g_pKernelModule->AddPropertyCallBack(self, NFrame::Player::Name(), this, &NFGameScene::OnObjectPropertyEvent);
			update(0.0f);
        }
...
int NFGameScene::OnObjectPropertyEvent(const NFGUID& self, const std::string& strPropertyName, const NFData& oldVar, const NFData& newVar)
{
	Sprite *pPlayer = m_Players.GetElement(self);
...
int NFGameScene::OnPlayerMoveEvent(const int nEventID, const NFDataList& varDataList)
{
...
int NFGameScene::OnPlayerChatEvent(const int nEventID, const NFDataList& varDataList)
{
	const NFGUID &self = varDataList.Object(0);
	Sprite *pPlayer = m_Players.GetElement(self);
